...
// We load the existing 5 problems from generator
const p1_problems = {};
...
add({
  ...
  walkthrough: {
    ...
    tableHeaders: ["left", "right", "nums[left]²", "nums[right]²", "Chosen", "idx", "res state"],
    tableRows: [
      ["0 (-4)", "4 (10)", "16", "100", "100 from right", "4", "[_, _, _, _, 100]"],
      ["0 (-4)", "3 (3)", "16", "9", "16 from left", "3", "[_, _, _, 16, 100]"],
      ["1 (-1)", "3 (3)", "1", "9", "9 from right", "2", "[_, _, 9, 16, 100]"],
      ["1 (-1)", "2 (0)", "1", "0", "1 from left", "1", "[_, 1, 9, 16, 100]"],
      ["2 (0)", "2 (0)", "0", "0", "0", "0", "[0, 1, 9, 16, 100] ✓"]
    ]
  },
  edgeCases: [
    { case: "nums = [-5, -3, -1]", expected: "[1, 9, 25]", explanation: "All negative inputs are reversed in magnitude." },
    { case: "nums = [1, 2, 3]", expected: "[1, 4, 9]", explanation: "All non-negative inputs preserve order naturally." },
    { case: "nums = [-2, 0, 2]", expected: "[0, 4, 4]", explanation: "Symmetric elements yield duplicate squares handled correctly." }
  ],
  code: {
    python: `class Solution:
...
add({
  ...
  walkthrough: {
    ...
    tableHeaders: ["i (fixed)", "nums[i]", "left", "right", "Sum", "Action"],
    tableRows: [
      ["0", "-4", "1 (-1)", "5 (2)", "-3", "Sum < 0 → left++ (no triplets with -4)"],
      ["1", "-1", "2 (-1)", "5 (2)", "0", "Match! [-1, -1, 2] recorded. left++, right--"],
      ["1", "-1", "3 (0)", "4 (1)", "0", "Match! [-1, 0, 1] recorded. left++, right--"],
      ["2", "-1", "-", "-", "-", "Duplicate nums[2] == nums[1] → skip"],
      ["3", "0", "4 (1)", "5 (2)", "3", "nums[3] > 0 or right exhausted → finish"]
    ]
  },
  edgeCases: [
    { case: "nums = [0, 0, 0]", expected: "[[0, 0, 0]]", explanation: "All zeroes produce exactly one valid zero triplet with duplicates skipped." },
    { case: "nums = [0, 1, 1]", expected: "[]", explanation: "No triplet sums to 0." },
    { case: "nums = [-2, 0, 1, 1, 2]", expected: "[[-2, 0, 2], [-2, 1, 1]]", explanation: "Multiple distinct triplets with duplicated values in input." }
  ],
  code: {
    python: `class Solution:
...
// Load existing and write
const arrayJsPath = path.join(__dirname, '../js/data/dsa/explanations/array.js');
let existing = {};
try {
  existing = require(arrayJsPath);
} catch (e) {
  console.log("No existing array.js or require failed, starting fresh.");
}

Object.assign(existing, p1_problems);
...
console.log("Successfully updated array.js! Total problems: " + Object.keys(existing).length);
